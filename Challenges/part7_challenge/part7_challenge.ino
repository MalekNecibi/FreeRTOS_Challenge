...
static SemaphoreHandle_t mutex_buffer;          // Printing to Serial buffer
static SemaphoreHandle_t count_sem_empty;       // How many buffer slots ready for new productions
static SemaphoreHandle_t count_sem_unread;      // How many productions not yet consumed
...
void producer(void *parameters) {
    ...
    for (int i = 0; i < num_writes; i++) {
        // wait for empty slot in shared buffer
        if ( xSemaphoreTake(count_sem_empty, portMAX_DELAY) ) {
            // Critical section (accessing shared buffer)
            if ( xSemaphoreTake(mutex_buffer, portMAX_DELAY) ) {
                buf[head] = num;
        ...
        // indicate to consumers that a new value is ready to be read in
        xSemaphoreGive(count_sem_unread);
    }
...
void consumer(void *parameters) {
    ...
    while (1) {
        ...
        // wait until a producer indicates that a new item is ready
        if ( xSemaphoreTake(count_sem_unread, portMAX_DELAY) ) {
            ...
            // inform producers that a new slot is available
            xSemaphoreGive(count_sem_empty);
...
void setup() {
    ...
    count_sem_empty = xSemaphoreCreateCounting(BUF_SIZE, BUF_SIZE);                 // all empty initially
    count_sem_unread = xSemaphoreCreateCounting(BUF_SIZE, 0);    // 
